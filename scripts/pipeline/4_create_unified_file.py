...
Domains: drugs, weapon (כל תיקי 5k נחשבים נשק — אין דומיין 5k בדטה).

סופי: לכל 8000+ התיקים בוצע ניסיון חילוץ מתחם עונש; תוצאות sentencing_unified
(drugs + weapon על החסרים) ממוזגות כאן (ממלאות מתחם רק כשעדיין חסר).
"""
...
DATA_DIR = NEW_TRY_DIR.parent / "data"  # Thesis!!!/data/
...
OUTPUT_DIR = BASE_DIR / "output"  # innovation_submission/output/

# ========== DATA SOURCES ==========
# Indictment facts
DRUGS_INDICTMENT_FACTS = DRUGS_DIR / "gpt" / "processed_verdicts_with_gpt.csv"
# data/drugs and data/drugs_3k (כוללים הכל)
DATA_DRUGS_INDICTMENT = DATA_DIR / "drugs" / "processed_verdicts_with_gpt.csv"
DATA_DRUGS_CITATIONS_DIR = DATA_DIR / "drugs" / "verdicts_tagged_citations"
DATA_DRUGS_3K_INDICTMENT = DATA_DIR / "drugs_3k" / "gpt" / "processed_verdicts_with_gpt.csv"
DATA_DRUGS_3K_CITATIONS_DIR = DATA_DIR / "drugs_3k" / "verdicts_tagged_citations"
SENTENCING_RANGE_DRUGS_3K = DATA_DIR / "drugs_3k" / "gpt_fewshot_classifier_extractor_results.csv"
...
DATA_5K_CITATIONS_DIR = DATA_DIR / "5k" / "verdicts_tagged_citations"  # מקור נוסף לציטוטי 5k
...
# סופי: חילוץ מתחם על כל החסרים (data_unified drugs + weapon)
SENTENCING_UNIFIED_DRUGS = OUTPUT_DIR / "sentencing_unified" / "gpt_fewshot_classifier_extractor_results_drugs_verdict_csv.csv"
SENTENCING_UNIFIED_WEAPON = OUTPUT_DIR / "sentencing_unified" / "gpt_fewshot_classifier_extractor_results_weapon_verdict_csv.csv"
...
def _read_csv_robust(path: Path) -> pd.DataFrame | None:
    """Read CSV with fallback encodings (utf-8, utf-8-sig, cp1255, latin-1). Returns None on failure."""
    for encoding in ("utf-8", "utf-8-sig", "cp1255", "latin-1", "iso-8859-8"):
        try:
...
def _normalize_verdict_id(s: str) -> str:
    """נרמול מזהה תיק להשוואה (ת\"פ→תפ, רווחים→קו תחתון)."""
    if pd.isna(s) or str(s).strip() == "":
        return ""
    s = str(s).strip()
    s = s.replace('ת"פ', 'תפ').replace('ע"פ', 'עפ').replace('ע"א', 'עא').replace('רע"פ', 'רעפ')
    return s.replace(" ", "_")


def load_citations(citations_dir: Path, also_register_normalized: bool = True) -> dict:
    """
    Load all citations from individual verdict files.
    Returns: {verdict_id: [(cited_case, citation_text, predicted_label), ...]}
    If also_register_normalized, indexes each file under both stem and normalize_id(stem) for lookup.
    """
    ...
    for csv_file in citations_dir.glob("*.csv"):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                entries.append(citation_entry)
            citations_dict[verdict_id] = entries
            if also_register_normalized:
                norm = _normalize_verdict_id(verdict_id)
                if norm and norm != verdict_id:
                    # avoid overwriting if another file already registered under norm
                    if not citations_dict[norm]:
                        citations_dict[norm] = list(entries)
        except Exception:
            skipped += 1
    if skipped > 0:
        print(f"   ⚠️ Skipped {skipped} citation CSV files (empty or encoding/parse errors)")
    
    return dict(citations_dict)


def merge_sentencing_fill_missing(existing: pd.DataFrame, new: pd.DataFrame) -> pd.DataFrame:
    """ממזג מקור חדש למתחמים: מוסיף/מעדכן רק כשבחדש יש range_low או range_high (ממלא חסרים)."""
    if new.empty:
        return existing
    if existing.empty:
        return new
    if "range_low" not in new.columns or "range_high" not in new.columns:
        return existing
    has_range = (
        (new["range_low"].notna() & new["range_low"].astype(str).str.strip().str.len().gt(0))
        | (new["range_high"].notna() & new["range_high"].astype(str).str.strip().str.len().gt(0))
    )
    new_with_range = new.loc[has_range].copy()
    if new_with_range.empty:
        return existing
    combined = pd.concat([existing, new_with_range], ignore_index=True)
    combined = combined.drop_duplicates(subset=["verdict"], keep="last")
    return combined
...
def load_citations_fallback(fallback_path: Path) -> dict:
    """Load citations_from_text_fallback.csv: verdict -> list of cited_case (from regex on verdict text)."""
    out = defaultdict(list)
    if not fallback_path.exists():
        return dict(out)
    try:
        df = pd.read_csv(fallback_path, encoding="utf-8-sig")
        if df.empty or "verdict" not in df.columns or "cited_case" not in df.columns:
            return dict(out)
        for _, r in df.iterrows():
            v, c = str(r["verdict"]).strip(), str(r.get("cited_case", "")).strip()
            if v and c:
                out[v].append(c)
        # dedupe per verdict
        return {v: list(dict.fromkeys(cits)) for v, cits in out.items()}
    except Exception:
        return {}
...
def create_unified_file(domain: str, 
                        ...
                        citations_fallback: dict | None = None) -> pd.DataFrame:
    """Create unified file for a domain. citations_fallback: verdict -> [cited_case] from backfill_citations_from_text."""
    ...
    for verdict in all_verdicts:
        ...
        # Add citations (as JSON string for easy reading); use fallback if no tagged citations
        verdict_citations = citations_dict.get(verdict, []) or citations_dict.get(_normalize_verdict_id(verdict), [])
        if not verdict_citations and citations_fallback and verdict in citations_fallback:
            verdict_citations = [{"cited_case": c, "citation_text": ""} for c in citations_fallback[verdict]]
...
def main():
    ...
    # Optional: citations extracted from verdict text for verdicts with 0 (backfill_citations_from_text.py)
    citations_fallback = load_citations_fallback(OUTPUT_DIR / "citations_from_text_fallback.csv")
    if citations_fallback:
        print(f"   📎 Loaded citations fallback for {len(citations_fallback)} verdicts (from text regex)")
    ...
    # סופי: מיזוג חילוץ מתחם על החסרים (sentencing_unified)
    if SENTENCING_UNIFIED_DRUGS.exists():
        drugs_unified_sent = load_sentencing_range(SENTENCING_UNIFIED_DRUGS)
        before = len(drugs_sentencing)
        drugs_sentencing = merge_sentencing_fill_missing(drugs_sentencing, drugs_unified_sent)
        print(f"   📌 Merged sentencing_unified drugs (fill missing ranges; {before} → {len(drugs_sentencing)} rows)")
    ...
    # Create unified file (new_try/drugs)
    drugs_unified = create_unified_file(
        ...
        output_path=OUTPUT_DIR / "drugs_unified_new_try.csv",
        citations_fallback=citations_fallback,
    )
    
    # ---------- data/drugs ----------
    print("\n   📂 Adding data/drugs...")
    data_drugs_indictment = load_indictment_facts(DATA_DRUGS_INDICTMENT)
    data_drugs_citations = load_citations(DATA_DRUGS_CITATIONS_DIR)
    if not data_drugs_indictment.empty:
        data_drugs_unified = create_unified_file(
            domain="drugs",
            indictment_facts_df=data_drugs_indictment,
            citations_dict=data_drugs_citations,
            sentencing_df=drugs_sentencing,
            target_verdicts=set(),
            output_path=OUTPUT_DIR / "drugs_unified_data_drugs.csv",
            citations_fallback=citations_fallback,
        )
    else:
        data_drugs_unified = pd.DataFrame()
    
    # ---------- data/drugs_3k ----------
    print("\n   📂 Adding data/drugs_3k...")
    data_drugs_3k_indictment = load_indictment_facts(DATA_DRUGS_3K_INDICTMENT)
    data_drugs_3k_citations = load_citations(DATA_DRUGS_3K_CITATIONS_DIR)
    data_drugs_3k_sentencing = load_sentencing_range(SENTENCING_RANGE_DRUGS_3K)
    if not data_drugs_3k_indictment.empty:
        data_drugs_3k_unified = create_unified_file(
            domain="drugs",
            indictment_facts_df=data_drugs_3k_indictment,
            citations_dict=data_drugs_3k_citations,
            sentencing_df=data_drugs_3k_sentencing,
            target_verdicts=set(),
            output_path=OUTPUT_DIR / "drugs_unified_data_drugs_3k.csv",
            citations_fallback=citations_fallback,
        )
    else:
        data_drugs_3k_unified = pd.DataFrame()
    
    # ---------- Merge all drugs (new_try + data/drugs + data/drugs_3k), no duplicate verdicts ----------
    to_combine = [drugs_unified]
    if not data_drugs_unified.empty:
        to_combine.append(data_drugs_unified)
    if not data_drugs_3k_unified.empty:
        to_combine.append(data_drugs_3k_unified)
    drugs_unified = pd.concat(to_combine, ignore_index=True)
    before_dedup = len(drugs_unified)
    # Prefer row with citations when same verdict appears in multiple sources (avoid losing citations from data_drugs_3k etc.)
    drugs_unified = drugs_unified.sort_values("citations_count", ascending=False)
    drugs_unified = drugs_unified.drop_duplicates(subset=["verdict"], keep="first")
    print(f"\n   ✅ Drugs combined: {before_dedup} rows → {len(drugs_unified)} unique verdicts (prefer row with citations)")
    drugs_unified.to_csv(OUTPUT_DIR / "drugs_unified.csv", index=False, encoding="utf-8-sig")
    ...
    # סופי: מיזוג חילוץ מתחם על החסרים (sentencing_unified)
    if SENTENCING_UNIFIED_WEAPON.exists():
        weapon_unified_sent = load_sentencing_range(SENTENCING_UNIFIED_WEAPON)
        before = len(weapon_sentencing)
        weapon_sentencing = merge_sentencing_fill_missing(weapon_sentencing, weapon_unified_sent)
        print(f"   📌 Merged sentencing_unified weapon (fill missing ranges; {before} → {len(weapon_sentencing)} rows)")
    ...
    # ========== 5K → WEAPON (כל תיקי 5k הם נשק; ממוזגים ל-weapon) ==========
    print("\n" + "=" * 40)
    print("📊 5K → נשק (מזג ל-weapon_unified)")
    print("=" * 40)
    
    five_k_indictment = load_indictment_facts(FIVE_K_INDICTMENT_FACTS)
    five_k_citations = load_citations(FIVE_K_CITATIONS_DIR)
    # טעינה גם מ-data/5k/verdicts_tagged_citations (כיסוי רחב יותר)
    if DATA_5K_CITATIONS_DIR.exists():
        data_5k_cit = load_citations(DATA_5K_CITATIONS_DIR)
        for vid, entries in data_5k_cit.items():
            five_k_citations.setdefault(vid, []).extend(entries)
        print(f"   📊 Loaded 5k citations: innovation_task + data/5k → {len(five_k_citations)} verdicts with citations")
    five_k_sentencing = load_sentencing_range(SENTENCING_RANGE_5K)
    print(f"   📊 Loaded {len(five_k_indictment)} indictment, {len(five_k_citations)} citations, {len(five_k_sentencing)} sentencing")
    
    five_k_unified = create_unified_file(
        domain="weapon",  # כל 5k = נשק — אין דומיין 5k
        indictment_facts_df=five_k_indictment,
        citations_dict=five_k_citations,
        sentencing_df=five_k_sentencing,
        target_verdicts=set(),
        output_path=None,  # לא שומרים קובץ נפרד 5k
        citations_fallback=citations_fallback,
    )
    # מיזוג לתוך weapon_unified (דדופ לפי verdict)
    if not five_k_unified.empty:
        weapon_unified = pd.concat([weapon_unified, five_k_unified], ignore_index=True)
        weapon_unified = weapon_unified.sort_values("citations_count", ascending=False)
        weapon_unified = weapon_unified.drop_duplicates(subset=["verdict"], keep="first")
        weapon_unified.to_csv(OUTPUT_DIR / "weapon_unified.csv", index=False, encoding="utf-8-sig")
        print(f"   ✅ Merged 5k into weapon_unified → {len(weapon_unified)} verdicts")
    ...
    combined = pd.concat([drugs_unified, weapon_unified], ignore_index=True)
