#!/usr/bin/env python3
"""
Supervised filter batch — score top-K Supervised-embedding neighbor pairs with
gpt-4.1 (V6 prompt), to enable a fair "+LLM" comparison vs citation/SimCSE.

Mirrors build_similarity_batch_simcse.py exactly (same model/prompts/parsing).

LEAKAGE-PREVENTION: candidates are restricted to (test_query × train_neighbor)
pairs. The test queries were held out from supervised training; the train
neighbors WERE in supervised training but the (test, train) PAIRS were never
seen during training (training pairs were train×train only).

Usage:
  python3 build_similarity_batch_supervised.py prepare [--k 20]
  python3 build_similarity_batch_supervised.py submit
  python3 build_similarity_batch_supervised.py status
  python3 build_similarity_batch_supervised.py process

Output: experiments/data_per_domain/similarity_batch_supervised/
"""
...
WORK = OUT / "similarity_batch_supervised"
...
# V6 score-only prompts — IDENTICAL to citation/simcse batch scripts
...
def build_supervised_pairs(K: int):
    """For each TEST query, top-K supervised neighbors restricted to TRAIN.
    Dedupe against existing LLM-scored pairs (140K + 49K)."""
    sup = {}
    for dom, ep, ip in [("drugs",
                          EXP/"simcse_outputs/supervised/verdict_embeddings_drugs.npy",
                          EXP/"simcse_outputs/supervised/verdict_index_drugs.csv"),
                        ("weapon",
                          EXP/"simcse_outputs/supervised/verdict_embeddings_weapon.npy",
                          EXP/"simcse_outputs/supervised/verdict_index_weapon.csv")]:
        emb = np.load(ep).astype(np.float32)
        idx = pd.read_csv(ip)
        sup[dom] = {"emb": emb, "idx": idx,
                    "v2i": dict(zip(idx.verdict, range(len(idx))))}

    domain_of = {}
    for dom in ["drugs","weapon"]:
        for v in sup[dom]["idx"].verdict:
            domain_of[v] = dom

    # Existing LLM scores (avoid re-scoring)
    existing = set()
    ...
    for dom in ["drugs", "weapon"]:
        ...
        print(f"  {dom}: train={len(train_ids)}, test={len(test_ids)}")
        for q in test_ids:
            qi = v2i[q]
            ...
            for j in order:
                tgt = train_ids[j]
                all_pairs.add(tuple(sorted([q, tgt])))

    new_pairs = all_pairs - existing
    print(f"  Supervised top-{K} unique pairs: {len(all_pairs):,}")
...
def cmd_prepare(args):
    print(f"[1] Building Supervised top-{args.k} (test × train) pairs...")
    pairs, domain_of = build_supervised_pairs(args.k)
...
def cmd_submit(args):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        b = cli.batches.create(input_file_id=up.id, endpoint="/v1/chat/completions",
                               ...
                               metadata={"description": f"supervised_topK_chunk{i}"})
...
def cmd_process(args):
    ...
    df.to_csv(RESULTS / "similarity_scores_supervised.csv", index=False)
    print(f"\n✅ saved {len(df):,} → {RESULTS/'similarity_scores_supervised.csv'}")
