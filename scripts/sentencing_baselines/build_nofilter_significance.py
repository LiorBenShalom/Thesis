#!/usr/bin/env python3
"""Build comparison + Wilcoxon for the no-filter pipeline."""
...
PRED_DIR = EXP / "data_per_domain/prediction_results/paper_style_nofilter"
...
def main():
    ...
    comp.to_csv(OUT_DIR / "comparison_nofilter.csv", index=False)
    print("=== COMPARISON (no citation filter, paper-style aggregation, THR=60-equiv, +σ) ===\n")
    ...
    for dom in ["drugs", "weapon"]:
        for tgt in ["low", "high"]:
            for sig in [False, True]:
                ...
                print()
    pd.concat(all_w, ignore_index=True).to_csv(OUT_DIR / "wilcoxon_nofilter.csv", index=False)
    print(f"All Wilcoxon -> {OUT_DIR/'wilcoxon_nofilter.csv'}")
