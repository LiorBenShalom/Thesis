...
PRED_DIR = EXP / "data_per_domain/prediction_results/qwk_thresholds"
...
def main():
    ...
    comp_self.to_csv(OUT_DIR / "comparison_qwk_self.csv", index=False)
    ...
    for dom in ["drugs","weapon"]:
        for tgt in ["low","high"]:
            for sig in [False, True]:
                ...
                print()
    pd.concat(all_w, ignore_index=True).to_csv(OUT_DIR / "wilcoxon_qwk.csv", index=False)
    print(f"\nAll Wilcoxon → {OUT_DIR/'wilcoxon_qwk.csv'}")
