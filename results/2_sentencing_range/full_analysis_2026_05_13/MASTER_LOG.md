...
## 4c. citation **בלי** LLM, ה-trade-off של 1-hop, ולמה הקורפוס (לא המחלץ) הוא הצוואר

נתונים: [`data/citation_only_vs_llm.csv`](data/citation_only_vs_llm.csv), [`data/citation_llm_rescue_examples.csv`](data/citation_llm_rescue_examples.csv). קוד: `scripts/citation_only_analysis.py`. `citation_only` = median של *כל* שכני-הציטוט (train, אותו fold, מתויגים), **ללא LLM** — אנלוג ל-sup_only.

### האם ה-LLM-rerank שווה את עצמו מעל מיצוע-ציטוט גולמי? (matched, אותן שאילתות)

| candidate set | domain | כיסוי | חציון שכנים | citation_only lo | llm_best (matched) | Δ(only−UB) | citation_llm (matched) | Δ(only−cl) |
|---|---|---|---|---|---|---|---|---|
| **1-hop בלבד** | drugs | 62% | 2 | 5.65 | 4.80 | +0.85 ✓ | 4.99 | +0.65 ✓ |
| | weapon | 62% | 2 | 11.39 | 10.91 | +0.48 **✗** | 11.24 | +0.16 **✗** |
| **full** (1+2hop+cocite) | drugs | 87% | 17 | 5.87 | 5.12 | +0.75 ✓ | 5.33 | +0.54 ✓ |
| | weapon | 95% | 33 | 13.70 | 11.96 | +1.74 ✓ | 12.39 | +1.32 ✓ |

(✓/✗ = מובהקות 95%CI + Wilcoxon. מספרים מלאים ב-CSV.)

**ה-trade-off המרכזי — ערך ה-LLM הוא לאפשר retrieval רחב בלי לאבד דיוק:**
- **1-hop ישיר מדויק אך דל-כיסוי**: weapon-1hop (11.39) **שקול סטטיסטית לתקרת llm_best** (p=0.52) — ציטוט ישיר הוא אות חזק כל-כך שמיצוע גולמי ≈ oracle. אבל כיסוי 62% בלבד, חציון **2 שכנים**.
- **ההרחבה (cocite/2hop) קונה כיסוי (95%) אך מזריקה רעש** (33 שכנים) → ואז ה-LLM-rerank נעשה **חיוני ומובהק מאוד** (weapon Δ=+1.74, p=1e-4). בלי LLM נאלצים לבחור בין רשת-צרה-מדויקת לרשת-רחבה-רועשת; ה-LLM שובר את ה-trade-off.

### למה ה-1-hop דל — **הקורפוס, לא המחלץ** (case study: תפ_4760-10-18)

השופט ציטט 7 תקדימים בנימוקי הענישה. בדיקת קובץ הציטוטים-המתויגים: **המחלץ תפס 7/7** (כולל מסווג-רלוונטיות `predicted_label`). edge נוצר רק אם התקדים קיים אצלנו כמסמך:

| תקדים | תויג? | בקורפוס? | ערכאה-ראשונה? | שמיש? |
|---|---|---|---|---|
| עפ 6886/17, עפ 5813/14, תפ 10649-07-11, תפ 40619-12-16 (בני-השוואה אמיתיים) | ✅ | רובם ❌ | ❌/ערעור | ❌ |
| תפ 33745-01-13 | ✅ | ✅ | ✅ | **✅ היחיד** |

מסקנה: ה-1-hop דל **לא** כי השיטה חלשה אלא כי (א) רוב התקדימים בני-ההשוואה (תיקי-בלדרי-קוקאין) פשוט **אינם בקורפוס**, (ב) רבים הם **ערעורים** ללא טווח ערכאה-ראשונה. עם קורפוס מלא, 1-hop היה חזק בהרבה; cocite+LLM הוא **פיצוי על קורפוס חלקי**.

### שתי דוגמאות שבהן ה-LLM הציל (ציטוטים לא-דומים)

| query | אמת low | citation_only (כל השכנים) | citation_llm (top-10 LLM) |
|---|---|---|---|
| תפח_20623-11-12 (נשק חמור) | 120 | 48 → שגיאה **72** | **120 → שגיאה 0** |
| תפ_4760-10-18 (סמים, 228 שכנים) | 42 | 12 → שגיאה 30 | 46 → שגיאה **4** |

המנגנון: co-citation מקשר תיקים שחולקים *אסמכתאות משפטיות* — לא חומרה. המיצוע נגרר ע"י תיקים זוטרים (ציון LLM 10–15) שנכנסו לרשת; ה-LLM קורא עובדות ובורר את הדומים-באמת. פירוט פר-שכן: `citation_llm_rescue_examples.csv`.

### Future-work בעבודה: חילוץ טווח מערעורים
**64% מקשתות-הציטוט מצביעות על ערעורים** (567 ערעורים מצוטטים ללא טווח). חילוץ-טווח לערעורים מעולם לא הושלם (95/1,745). batch חילוץ (480 ערעורים, gpt-4.1, פרומפט מותאם-ערעור שתוקף על עפ_6886-17→דיוק מושלם) **רץ כעת**; התוצאה תימדד כשיפור-כיסוי ל-citation. ראה `experiments/data_per_domain/appeal_range_extraction/`.
