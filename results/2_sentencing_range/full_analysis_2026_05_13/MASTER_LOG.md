...
ה"רעש" הטבעי — ההפרש הצפוי בין שני תיקים אקראיים (50,000 זוגות אקראיים, seed=42):
...
- *מה*: ההפרש הצפוי בין 2 verdicts אקראיים.
- *איך*: לכל domain — דגימת `50,000` זוגות אקראיים (i,j) (`np.random.default_rng(seed=42)`, דטרמיניסטי), חישוב d_lo/d_hi לכל זוג, ממוצע.
- *כמה*: n=50,000 זוגות לכל domain.
