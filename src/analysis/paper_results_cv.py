...
MODELS = ["gpt4","gpt5mini","gpt52","gpt51_thinking","claude_sonnet_4_6",
          "gemini_25_pro","gemini_3_flash","gemma3_27b","llama3_70b","qwen3_235b","qwen_hf"]
TASKS = [0, 1]

from sklearn.model_selection import StratifiedKFold

def best_f1(scores: np.ndarray, y: np.ndarray, seed: int = 42) -> float:
    """5-fold Stratified CV F1: threshold picked on train fold, evaluated on test fold, averaged."""
    if len(np.unique(y)) < 2: return np.nan
    if y.sum() < 5 or (len(y) - y.sum()) < 5: return np.nan
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=seed)
    fold_f1s = []
    for tr_idx, te_idx in skf.split(scores, y):
        tr_s, tr_y = scores[tr_idx], y[tr_idx]
        te_s, te_y = scores[te_idx], y[te_idx]
        best_t, best_f = 0.0, 0.0
        for thr in np.unique(tr_s):
            f = f1_score(tr_y, (tr_s >= thr).astype(int), zero_division=0)
            if f > best_f:
                best_f, best_t = f, thr
        fold_f1s.append(f1_score(te_y, (te_s >= best_t).astype(int), zero_division=0))
    return float(np.mean(fold_f1s))

def cell_metrics(base: Path, rep_prefix: str, model: str, task: int) -> tuple[float,float]:
    p = base/f"{rep_prefix}_v6score_{model}_binary_0_preds.csv"
    if not p.exists(): return np.nan, np.nan
    df = pd.read_csv(p)
    df = df[df["status"] == "ok"] if "status" in df.columns else df
    if len(df) < 20: return np.nan, np.nan
...
for dom, base in DOMAINS.items():
    for rep, pref in REP_PREFIX.items():
        for m in MODELS:
            for t in TASKS:
                f1, ap = cell_metrics(base, pref, m, t)
                rows.append(dict(domain=dom, rep=rep, model=m, task=t, F1=f1, AP_PR=ap))
...
md.append("_Metrics: 5-fold stratified CV F1 (threshold picked on train fold); AP-PR (average precision, threshold-free)._")
...
md.append("## 1. Summary — mean F1 across 11 models\n")
...
for dom in DOMAINS:
    for t in TASKS:
        focus = "Manual" if t==0 else "Manual"  # Manual is our main representation for both
        md.append(f"\n### {dom.upper()} — Task b{t} — focus = {focus}")
        md.append(f"\n**F1**\n"+md_table(focus_table(sig_f1, focus, dom, t)))
        md.append(f"\n**AP-PR**\n"+md_table(focus_table(sig_ap, focus, dom, t)))

md.append("\n## 5. Full per-cell results\n")
md.append("See `full_results.csv` (one row per rep × model × domain × task).\n")
